...
const SkillsSection = () => {
    return (
        <section id="skills" className="min-h-screen flex items-center justify-center py-20 gradient-bg px-6 lg:px-24">
                                        ...
                                        <Tooltip key={skill.name}>
                                            <TooltipTrigger asChild>
                                                <motion.div
                                                    ...
                                                    transition={{ duration: 0.4, delay: skillIndex * 0.08, ease: 'easeOut' }}
                                                    viewport={{ once: true }}
                                                    className="flex flex-col items-center space-y-2 cursor-pointer group"
                                                >
                                                    <motion.div
                                                        whileHover={{ scale: 1.1, y: -5, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}
                                                        whileTap={{ scale: 0.97 }}
                                                ...
                                                <div className="text-center">
                                                    <div className="flex justify-center items-center space-x-1">
                                                        ...
                                                        {[0, 1, 2, 3, 4].map((i) => {
                                                            ...
                                                            const half = !full && i < skill.level && skill.level % 1 !== 0;
                                                            return (
                                                                <Star key={i} variant={full ? 'full' : half ? 'half' : 'empty'} />
                                                            );
                                                        })}
                                                    </div>
                                                </div>
